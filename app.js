...
// VIN KEYPAD
// ============================
// Row layout mirrors a standard QWERTY keyboard (digit row on top, then the
// three letter rows) instead of a flat alphabetical grid — the old A-Z order
// was slow to scan under pressure. I/O/Q are real keys here, not omitted;
// applyVinKeypadRestriction() disables them only when the active target is a
// genuine VIN field (see VIN_SUGGEST_TARGETS below), since a VIN never
// contains them but other targets — e.g. the RECORDS search box, which also
// matches make/model/notes — still need the full alphabet.
...
function buildVinKeypad() {
  ...
  // Layout (each row is its own flex container, see .vin-key-row in app.css):
  //   1  2  3  4  5  6  7  8  9  0
  //   Q  W  E  R  T  Y  U  I  O  P
  //   A  S  D  F  G  H  J  K  L
  ...
  const keyBtn = (k) => {
    ...
    return `<button class="vin-key ${typeClass}" type="button" onclick="vinKeypadType('${k}')"${restrictAttr}>${k}</button>`;
  };

  let html = "";
  VIN_KEYPAD_ROWS.forEach((row, i) => {
    html += '<div class="vin-key-row">';
    row.forEach((k) => { html += keyBtn(k); });
    if (i === VIN_KEYPAD_ROWS.length - 1) {
